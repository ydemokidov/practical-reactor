...
public class c5_CreatingSequence {
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Flux.from(Mono.fromCallable(counter::incrementAndGet)).repeat(9);
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(() -> 1,(objects, integerSynchronousSink) -> {
            integerSynchronousSink.next(objects);
            if(objects==5) integerSynchronousSink.complete();
            return ++objects;
        });
